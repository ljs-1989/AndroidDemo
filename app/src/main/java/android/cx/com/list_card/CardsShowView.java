...
public class CardsShowView extends RelativeLayout implements View.OnTouchListener{
    ...
    public CardsShowView(@NonNull Context context, @Nullable AttributeSet attrs, @AttrRes int defStyleAttr) {
        super(context, attrs, defStyleAttr);
         mContext = context;
        CLOSE_ITEM_SPACE = (int)convertDpToPixel(7,context);
       ...
       this.post(new Runnable() {
            ...
            public void run() {
                ...
                try{
                    ...
                    marginTop = ((LayoutParams)getLayoutParams()).topMargin;
    ...
    private void addAllCardsView(){
        ...
        for(int i = 0;i< childList.size();i++){
            RelativeLayout.LayoutParams layoutParams = new LayoutParams(600,260);
            layoutParams.topMargin = CLOSE_ITEM_SPACE*2- CLOSE_ITEM_SPACE*i+ getPaddingTop() + marginTop;
            childList.get(i).setId(i);
    ...
    private OnClickListener clickListener = new OnClickListener() {
        ...
        public void onClick(View v) {
                     if(viewStatus == ITEM_OPEN){
                         ...
                         stateChangeListener.stateChange(ITEM_CLOSE);
                         //先收起item
                         closeItemView(v.getId());
                         viewStatus = ITEM_CLOSE;
                     }else {
                         if(stateChangeListener!=null)
                         stateChangeListener.stateChange(ITEM_OPEN);
                         requestLayout();
                         openItemView();
    ...
    public boolean canScrollUp(){
        ...
        if(getChildCount()>0&&openContentHeight>viewHeight){
            canScrollUp =  (openContentHeight-getScrollY()) - (viewHeight-getPaddingTop()-getPaddingBottom()) > 0;
    ...
    private void openItemView(){
        for(int i=1;i<childList.size();i++){
            int startY = CLOSE_ITEM_SPACE*(childList.size() - i) + getPaddingTop();
     ...
     * @param clickViewId
     */
    private void closeItemView(final int clickViewId){
        for(int i=childList.size()-1;i>0;i--){
            ...
            if(i==1){
                objectAnimator.addListener(new AnimatorListenerAdapter() {
                    ...
                    public void onAnimationEnd(Animator animation) {
                        //重新排序
                        if(clickViewId!=childList.size()-1){
                            View  clickView =  childList.get(clickViewId);
